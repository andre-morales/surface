...
std::optional<Vector3f> Collisions::raycastTriangle(const Ray& ray, const Vector3f triangle[]) {
	...
	if (pointInTriangle(*hit, triangle)) {
		return hit;
	}
...
bool Collisions::pointInTriangle(const Vector3f& point, const Vector3f triangle[]) {
...
/*bool Collisions::raycastPlane(Ray ray, Plane plane, Math::Vec3f* hitPoint) {
	float nd = Math::dot(ray.direction, plane.direction);
	float pn = Math::dot(ray.position, plane.direction);
